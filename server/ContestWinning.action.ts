...
import { contestWinningCreateOrUpdateSchema } from "@/app/[locale]/dashboard/contest-winning/[page]/types";

const logger = new Logger({ name: "contest-winning" });
...
export const deleteContestWinning = AdminRequestProvider.input(
  z.object({
    id: z.number(),
  })
)
  ...
  .handler(async ({ input, ctx }) => {
    try {
      await prisma.contestWinning.delete({
        where: {
          id: input.id,
        },
      });
...
export const createOrUpdateContestWinning = AdminRequestProvider.input(
  ...
  .handler(async ({ input, ctx }) => {
    try {
      if (!!input.id) {
        ...
        await prisma.contestWinning.update({
          where: {
            id: input.id,
          },
          data: {
        ...
        await prisma.contestWinning.create({
          data: {
            ...input,
            page: {
              connect: input.page?.map((p) => ({
