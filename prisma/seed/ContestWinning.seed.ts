...
export async function ContestWinningSeeder() {
  await prisma.contestWinning.deleteMany({ where: { id: { not: 0 } } });

  const data: Prisma.ContestWinningCreateInput[] = [
    {
  ...
  const logger = new Logger({ name: "contestWinning" });
  logger.info(`success :✅ ${contestWinning.length} contestWinning was seeded`);
}
