...
import {
  ...
  CollaborationCreateOrUpdateType,
  PageIdSlugType,
} from "../[locale]/dashboard/collaborations/[page]/types";
...
import {
  createOrUpdateCollaboration,
  deleteCollaboration,
} from "@/server/collaboration.action";
...
export default function ExpandableCollaborationsCardDemo({
  cards,
  locale,
  pages,
}: ExpandableCardDemoProps) {
  const t = useClientTranslation("collaboration-dashboard");
  const [active, setActive] = useState<Collaboration | boolean | null>(null);
  const [isExpanded, setIsExpanded] = useState<boolean>(false);
  const [file, setFile] = useState<FileType | null>(null);

  const defaultValues: CollaborationCreateOrUpdateType = {
    id: undefined,
    title: "",
    description: "",
    picture: "",
  ...
  } = useForm<CollaborationCreateOrUpdateType>({
    resolver: zodResolver(CollaborationCreateOrUpdateSchema),
  ...
  const onSubmitForm: SubmitHandler<CollaborationCreateOrUpdateType> = async (
    data
  ) => {
    ...
    try {
      ...
      const result = await createOrUpdateCollaboration(data).then(
        (res) => res[0]
      );
      if (result) {
  ...
  const handleDelete = async ({ id }: { id: number }) => {
    try {
      const isConfirm = await confirm();
      if (!isConfirm) return;
      const result = await deleteCollaboration({
        id,
      }).then((res) => res[0]);
      if (result) {
        await successDialog().then(() => window.location.reload());
  ...
  const handleExpend = () => {
    if (!isExpanded && active != null && typeof active === "object") {
      Object.keys(active).forEach((key) => {
        setValue(
          key as keyof CollaborationCreateOrUpdateType,
          (active as CollaborationCreateOrUpdateType)[
            key as keyof CollaborationCreateOrUpdateType
          ]
        );
      });
    ...
    <>
      ...
      <motion.div className="flex justify-start items-center gap-4 mx-auto w-full max-w-2xl">
        <Button
          ...
          onClick={() => {
            handleExpend();
            setActive(defaultValues as Collaboration);
          }}
